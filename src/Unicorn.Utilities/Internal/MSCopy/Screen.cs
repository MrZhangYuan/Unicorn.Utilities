...
namespace Unicorn.Utilities.Internal.MSCopy
{
    public static class Screen
    {
        internal static void FindMaximumSingleMonitorRectangle(RECT windowRect, out RECT screenSubRect, out RECT monitorRect)
        {
            ...
            NativeMethods.EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, delegate (IntPtr hMonitor, IntPtr hdcMonitor, ref RECT rect, IntPtr lpData)
            {
        ...
        {
            IntPtr intPtr = NativeMethods.MonitorFromPoint(new POINT
            {
                X = (int)point.X,
                Y = (int)point.Y
            }, 2);
